...
function formatWFULearningMaterials(content: string, context: TemplateContext = {}): string {
  ...
  // Check for headings *with* markdown stars, so we don't need to strip them first.
  ...
  let bodyHtml = '';
  let currentListType: 'readings' | 'videos' | 'generic' | null = null;
  let inUl = false;
  let pendingVideoTitle: string | null = null;

  const openUl = () => { if (!inUl) { bodyHtml += '<ul>\n'; inUl = true; } };
  const closeUl = () => { if (inUl) { bodyHtml += '</ul>\n'; inUl = false; } };
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Ignore empty lines or lines that are ONLY markdown characters
    if (!line || /^[_*]+$/.test(line)) { 
      continue; 
    }

    // Check for H3 (Required/Optional Resources)
    if (h3Names.some(re => isSection(line, re))) {
      closeUl();
      inTextbookReadings = false;
      ...
      // --- UPDATED LOGIC ---
      const heading = line
        .replace(/\*\*|__/g, '') // 1. Remove markdown
        .trim()                  // 2. Trim whitespace
        .replace(/:$/, '');      // 3. Remove trailing colon
    ...
    // Check for H4 (Readings, Videos, etc.) OR a generic heading (ends in colon)
    const isGenericH4 = /.+:\s*$/.test(line); // Any line ending in ':'
    if (h4Names.some(re => isSection(line, re)) || isGenericH4) {
      ...
      const cleanName = name; // Already clean
      
      // Special: Textbook Readings always <h4> and start a new readings list
      if (/^(\*\*|__)*Textbook Readings:?(\*\*|__)*$/i.test(line)) { // Check original line
        bodyHtml += `<h4>Textbook Readings</h4>\n`;
        inTextbookReadings = true;
        bodyHtml += '<ul>\n';
        readingsIndentStack = [0];
        continue;
      }
      
      bodyHtml += `<h4>${cleanName}</h4>\n`;
      currentListType = /^(\*\*|__)*Video/i.test(line) ? 'videos' : /^(\*\*|__)*Reading/i.test(line) ? 'readings' : 'generic';
      continue;
    }

    // Textbook Readings block: all lines until next heading become nested <li>
    if (inTextbookReadings) {
      const indent = raw.match(/^\s*/)?.[0].length || 0;
      // If line looks like a heading, close readings
      if (h3Names.some(re => isSection(line, re)) || h4Names.some(re => isSection(line, re)) || /.+:\s*$/.test(line)) {
        while (readingsIndentStack.length > 1) { bodyHtml += '</ul>\n'; readingsIndentStack.pop(); }
        bodyHtml += '</ul>\n';
        inTextbookReadings = false;
        // Reprocess this line as a heading
        i--;
        continue;
      }
      // Adjust nesting based on indent
      while (readingsIndentStack.length > 1 && indent < readingsIndentStack[readingsIndentStack.length - 1]) {
        bodyHtml += '</ul>\n';
        readingsIndentStack.pop();
      }
      if (indent > readingsIndentStack[readingsIndentStack.length - 1]) {
        bodyHtml += '<ul>\n';
        readingsIndentStack.push(indent);
      }
      // Remove dash/bullet and wrap as <li>
      const clean = line.replace(/^[-•\*]\s*/, '');
      bodyHtml += `<li>${clean}</li>\n`;
      continue;
    }

    // Videos section handling (includes YouTube, TED, and other video URLs)
    if (currentListType === 'videos') {
      closeUl();
      // Check if this line contains a YouTube or TED URL (with or without a title prefix)
      const ytMatch = line.match(youtubeRe);
      const tedMatch = line.match(tedRe);
      if (ytMatch || tedMatch) {
        // Extract title if present (pattern: "Title https://...")
      ...
      // If this line is a non-YT/TED URL, finalize with placeholder under the pending title
      if (isBareUrl(line)) {
        if (pendingVideoTitle) {
          bodyHtml += `<p>${pendingVideoTitle}</p>\n`;
          bodyHtml += `<div class=\"WFU-Container-LectureMedia\">\n                <div class=\"VideoPlayer\">\n                    <p>HERE</p>\n                </div>\n            </div>\n`;
    ...
    // Readings list items (default inside Resources or explicit Readings section)
    if (currentListType === 'readings') {
      // Clean line by removing list bullets
      const cleanLine = line.replace(/^[-•\*]\s*/, '');
      // Detect "Title URL" pattern to hyperlink the title
      const m = cleanLine.match(/^(.*?)\s+(https?:\/\/\S+)\s*$/);
      openUl();
      if (m) {
    ...
    // Generic resource sections ('generic' list type) list item transformation
    if (currentListType === 'generic') {
        const cleanLine = line.replace(/^[-•\*]\s*/, '');
        // Patterns: Title URL: description  |  Title URL
        const patWithDesc = /^(.*?)\s+(https?:\/\/\S+):(.*)$/;
        const patSimple = /^(.*?)\s+(https?:\/\/\S+)\s*$/;
        const mDesc = cleanLine.match(patWithDesc);
        const mSimple = !mDesc ? cleanLine.match(patSimple) : null;
        ...
        if (mDesc) {
            ...
            bodyHtml += `<li><a href="${rawUrl}" target="_blank" rel="noopener">${rawTitle}</a>${rest ? `: ${rest}` : ''}</li>\n`;
            continue;
        }
        ...
        if (mSimple) {
            ...
            bodyHtml += `<li><a href="${rawUrl}" target="_blank" rel="noopener">${rawTitle}</a></li>\n`;
            continue;
        }
        
        // Non-URL line inside generic section: treat as paragraph (close list first)
        if (!isBareUrl(cleanLine)) {
            closeUl();
            bodyHtml += `<p>${autolink(cleanLine)}</p>\n`;
            continue;
        }
        // Bare URL alone: show as list item with URL as both title and href
        if (isBareUrl(cleanLine)) {
            openUl();
            bodyHtml += `<li><a href="${cleanLine}" target="_blank" rel="noopener">${cleanLine}</a></li>\n`;
            continue;
        }
    }
    
    // Fallback: paragraphs with autolinked URLs
...
function formatWFUCourseWelcome(content: string, context: TemplateContext = {}): string {
  const courseName = context.courseName || 'Incident Management and Business Continuity';
  const courseCode = context.courseCode || 'CYB 720';
  // Normalize baseUrl and provide safe default; ensure no trailing slash to avoid //courses
  const baseUrl = ((context.baseUrl && String(context.baseUrl).trim()) || 'https://wakeforest.instructure.com').replace(/\/+$/, '');
  const courseId = (context.courseId ? String(context.courseId) : '77445').trim();
  
  // Get module titles from context (array of 8 titles) or use defaults
    // Accept moduleTitles as a single string (textarea, one per line) or array
    let moduleTitles: string[] = [];
    if (typeof context.moduleTitles === 'string') {
      moduleTitles = context.moduleTitles.split('\n').map(t => t.trim()).filter(Boolean);
    } else if (Array.isArray(context.moduleTitles)) {
      moduleTitles = context.moduleTitles.map(t => (t || '').trim()).filter(Boolean);
    }
    // Replace purely numeric entries with placeholder 'Title'
    moduleTitles = moduleTitles.map(t => (/^\d+$/.test(t) ? 'Title' : t || 'Title'));
    // Always output 8 modules, fill with 'Title' if not enough
    while (moduleTitles.length < 8) moduleTitles.push('Title');
    moduleTitles = moduleTitles.slice(0, 8);
  
  // Build module IDs (starting from a base, incrementing by 1)
  const baseModuleId = 258871;
  const moduleLinks = moduleTitles.map((title, index) => {
    const moduleNum = index + 1;
    const moduleId = baseModuleId + index;
    return `<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12"><a title="Module ${moduleNum}" href="${baseUrl}/courses/${courseId}/modules/${moduleId}?wrap=1">Module ${moduleNum}: ${title}</a></div>`;
  }).join('\n            ');

  // Strip all HTML tags from content for plain text only
